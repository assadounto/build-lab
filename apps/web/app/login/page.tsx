...
function SignIn(){
  ...
  return <main className="container auth-page"><div className="auth-intro"><span className="eyebrow">WELCOME BACK</span><h1>Good ideas grow when you keep building<span className="orange-dot">.</span></h1><p>Sign in to pick up your project, record your progress and keep every milestone together.</p><Link className="text-link" href="/projects">Explore projects first →</Link></div><form className="panel auth-form" onSubmit={submit}><h2>Sign in to BuildLab</h2><p>Use your invited BuildLab account.</p><label>Email address<input type="email" autoComplete="email" value={email} onChange={event=>setEmail(event.target.value)} required/></label><label>Password<input type="password" autoComplete="current-password" value={password} onChange={event=>setPassword(event.target.value)} required/></label>{error&&<p className="form-error" role="alert">{error}</p>}<button type="submit" className="button" disabled={busy}>{busy?"Signing in…":"Sign in →"}</button><small>Self-service registration is coming later.</small></form></main>
}
