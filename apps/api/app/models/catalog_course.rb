class CatalogCourse < ApplicationRecord
  ...
  def as_catalog_json
    { id: id, title: title, slug: slug, summary: summary, description: description, level: level, hours: hours, image_url: image_url, status: status, category: catalog_category.as_catalog_json, related_project_slug: related_project_slug, lessons: catalog_lessons.map(&:as_catalog_json), created_at: created_at }
  end
